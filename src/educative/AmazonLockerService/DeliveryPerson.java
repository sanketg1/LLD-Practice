...
public class DeliveryPerson {
    ...
    public void deliverPackage(LockerPackage pkg,Locker locker){
        if(locker.addPackage(pkg)){
            System.out.println("DeliveryPerson" + deliveryPersonId+"delivered package"+pkg.getPackageId()+"to locker"+locker.getLockerId());
        }
    }

    public void pickupReturn(LockerPackage pkg,Locker locker){
        if(locker.removePackage(pkg.getCode())){
            System.out.println("DeliveryPerson" + deliveryPersonId+"pick up returned package"+pkg.getPackageId()+"from locker"+locker.getLockerId());
        }
    }
